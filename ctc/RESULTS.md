...
unreliable and naming the generator's word draw as the first thing to fix.
`SYNTH_V2_DESIGN.md` then measured that v1 synthesis is **90.4 % separable** from
real traces on its speed profile alone, and `SYNTH_V2_RESEARCH_AUDIT.md` built
the proposed fixes as prototypes and scored them before endorsing any. Phase P
ships the union: a **wordfreq token-mass word draw**, **geometry-matched donor
selection**, **vertex-aligned per-segment re-timing** at the measured human
isochrony exponent α = 0.5, and a new **acquisition-bandwidth matching** stage
that pushes each synthetic path back through the real 60 Hz chain at a duration
predicted by a law fit on MIT English only.

**The ship gate.** Real Yandex valid-10k, eval-only footing, 8,471 in-dict rows,
app CKDT preset, exported fp32 graph:

| arm | in-dict t1 | ≤3 | ≥4 | greedy |
|---|---|---|---|---|
| `ru_synth_ch80` — the v1 baseline | 77.42 | 86.47 | 71.70 | 37.07 |
| v1 control at Phase-O's donor footing | 75.73 | 83.66 | 70.71 | 31.34 |
| **`ru_synth_v2_ch80` — SHIP** | **79.73** | 85.77 | **75.92** | **56.12** |

**+2.31, paired McNemar p = 2.6e-09**, inside the pre-registered +2…+5 band; at
matched donor footing the generator change alone is **+3.14, p = 6.4e-14**. The
mechanism is the emissions: **greedy CTC on real Russian goes 37.07 → 56.12**
without a single real Cyrillic row in training. Almost all of it lands on the
≥4-letter stratum (+4.22, p = 3.6e-17), which is 61 % of real usage.

**The gate battery.** G1 endpoints in band with the wrong-geometry control at
0.02 · G2 length mix within 0.001 of the wordfreq token mass (from 3.3 % ≤3 words
to 26.9 %) · G3 every amended bar met, with step_cv KS 0.597 → 0.108, sharp_turns
0.443 → 0.306 and lag-1 speed autocorrelation — the largest single-statistic gap
in the campaign — 0.618 → 0.067 · G4 gap-closure 36.0 % (MLP speed), 22.6 %
(GBM), **84.2 % on the English→English footing**, against a *measured* 0.50 floor
and a permutation null at 0.4999.

**Two things recorded as missed.** The G5 short-word corollary (85.77 against a
bar of 86.4, though the paired test cannot distinguish the regression from zero,
p = 0.27, and the whole of it is carried by a donor-footing term rather than by
v2). And the G4 *standard* UCL₉₅ ≤ 0.60, which is not met and cannot be: the
English→English control puts ≈0.19 of the residual separability on the donor
bank's language, which no generator change can reach.

**A measured cost nobody had priced.** Phase O's 90/10 donor split — introduced
so the synthesis holdout would be donor-disjoint, and never trained on — costs
**−1.69 real top-1 and −5.7 greedy** (p = 5.2e-07). `script_synth --generator v1
--train-donor-side all` reproduces Phase I-B's cache bit-exactly, which is what
makes that attribution airtight.

**Phase O's registered question, answered.** On the v2 holdout the ru model beats
the English ch192 zero-shot by **+3.87** where real data says **+3.41** — the sign
inversion is gone and the magnitudes agree to half a point (Phase O read −2.28
against +1.09). All five corpus-less scripts show the same flip against the same
control, and Hebrew clears the ≥70 gate it failed (65.36 → 77.00 on its own v2
holdout). Those five numbers remain **generator-relative** — a v2 holdout is
generated by v2 — and are not accuracy claims for Greek, Ukrainian, Bulgarian,
Macedonian or Hebrew swipe typing.

**P6, the closing round** (`PHASE_P.md` §8): the five corpus-less scripts were
regenerated and retrained on the **full donor pool** — the ru ship arm's own
footing, worth **+0.86 real top-1** there — and on their own holdouts, which are
provably the same 10,000 rows, the change reads **−0.09 pooled over 50,000
paired rows, p = 0.44**. The new bytes ship (`*_synth_v2full_ch80*`, he's parity
flag retired at 4.04e-04) on the strength of ru's real measurement, not the
holdout's; and the null itself is the phase's last piece of evidence that a
synthesis holdout cannot see what real swipes can.
