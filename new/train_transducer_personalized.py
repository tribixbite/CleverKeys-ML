...
try:
    ...
    print("Warning: data_augmentation module not available. Augmentation features will be disabled.")
...
try:
    ...
    print("Warning: progressive_unfreezing module not available. Unfreezing features will be disabled.")
...
CONFIG: Dict[str, Any] = {
    ...
    "training": {
        "batch_size": 320,          # Maximize GPU utilization on a 16GB+ card. Larger batches provide more stable gradients.
...
def _has_usable_cuda() -> bool:
    ...
    try:
        ...
        if torch.cuda.device_count() <= 0: return False
...
def load_key_centers(path: Optional[str]) -> List[Tuple[str, float, float]]:
    ...
    if path:
        try:
            ...
            print(f"Warning: Could not load key centers from {path}. Using default. Error: {e}")
...
class PersonalizedSwipeFeaturizer:
    ...
    def _compute_feature_vector(self, points: List[Dict[str, float]], idx: int) -> np.ndarray:
        ...
        if prev and prev2:
            ...
            dt2 = max((float(prev.get("t", 0.0)) - float(prev2.get("t", 0.0))) / 1000.0, 1e-6)
        ...
        key_distances = sorted([math.hypot(x - kx, y - ky) for _, kx, ky in self.key_centers])[:5]
        while len(key_distances) < 5: key_distances.append(1.0)
        ...
        if len(win_pts) > 1:
            ...
            win_mean_x, win_std_x, win_mean_y, win_std_y, win_range_x, win_range_y = x, 0.0, y, 0.0, 0.0, 0.0
        ...
        feature_dict = {
            ...
            'dist_key2': key_distances[1], 'dist_key3': key_distances[2], 'dist_key4': key_distances[3],
            'dist_key5': key_distances[4], 'progress': progress, 'is_start': is_start, 'is_end': is_end,
...
class PersonalizedSwipeDataset(Dataset):
    ...
    def compute_sampling_weights(self, sampling_cfg: Dict[str, Any]) -> Optional[np.ndarray]:
        ...
        max_len = int(sampling_cfg.get("max_word_length", 99)) if sampling_cfg.get("max_word_length") else 99
        max_freq = int(sampling_cfg.get("max_frequency", 999999)) if sampling_cfg.get("max_frequency") else 999999
        ...
        if not weights or sum(weights) == 0: return None
        ...
        try:
            with torch.no_grad():
                ...
                for ref, pred in zip(references, predictions):
                    pred_text = pred[0].text if isinstance(pred, list) and pred else str(pred)
                    if pred_text != ref:
                        print(f"\033[1;33m[{stage} mispred]\033[0m ref='{ref}' pred='{pred_text}'")
                        return # Log only the first error per batch
        except Exception as e:
            print(f"Could not log batch errors: {e}")

    def validation_step(self, *args, **kwargs):
        with torch.cuda.amp.autocast(enabled=False): # Run validation in fp32 for stability
...
def build_dataloaders(cfg: DictConfig, vocab: Dict[str, int]):
    ...
    train_ds = PersonalizedSwipeDataset(
        manifest_path=cfg.data.train_manifest, vocab=vocab, max_trace_len=cfg.data.max_trace_len,
        preprocess_cfg=cfg.preprocess, featurizer=featurizer, augmenter=augmenter, is_training=True
    )
    val_ds = PersonalizedSwipeDataset(
        manifest_path=cfg.data.val_manifest, vocab=vocab, max_trace_len=cfg.data.max_trace_len,
        preprocess_cfg=cfg.preprocess, featurizer=featurizer, augmenter=None, is_training=False
    )
    ...
    if train_weights is not None:
        train_sampler = WeightedRandomSampler(
            weights=torch.from_numpy(train_weights), num_samples=len(train_weights), replacement=True
        )
        print(f"Enabled '{cfg.sampling.strategy}' sampling (weight range {train_weights.min():.3f}–{train_weights.max():.3f})")

    train_loader = DataLoader(
        dataset=train_ds, batch_size=cfg.training.batch_size, sampler=train_sampler,
        shuffle=train_sampler is None, num_workers=cfg.training.num_workers,
        collate_fn=collate_fn, pin_memory=_has_usable_cuda(), drop_last=True,
        persistent_workers=cfg.training.num_workers > 0, prefetch_factor=4 if cfg.training.num_workers > 0 else 2
    )

    val_sampler = None
    val_weights = val_ds.compute_sampling_weights(cfg.validation)
    if val_weights is not None:
        num_samples = min(len(val_weights), int(cfg.validation.get('max_samples', len(val_weights))))
        val_sampler = WeightedRandomSampler(
            weights=torch.from_numpy(val_weights), num_samples=num_samples, replacement=False
        )
    ...
    val_loader = DataLoader(
        dataset=val_ds, batch_size=val_batch_size, sampler=val_sampler,
        shuffle=val_sampler is None, num_workers=val_workers, collate_fn=collate_fn,
        pin_memory=_has_usable_cuda(), drop_last=False, # drop_last=False is important for validation
        persistent_workers=val_workers > 0, prefetch_factor=2 if val_workers > 0 else None
    )
...
def build_model_config(cfg: DictConfig, labels: List[str]) -> DictConfig:
    ...
    return DictConfig({
        'labels': labels, 'sample_rate': 16000,
        'model_defaults': {'enc_hidden': cfg.model.encoder.d_model, 'pred_hidden': cfg.model.decoder.pred_hidden},
        'preprocessor': {
            '_target_': 'nemo.collections.asr.modules.AudioToMelSpectrogramPreprocessor',
            'features': cfg.model.encoder.feat_in, 'normalize': 'per_feature',
            'sample_rate': 16000, 'window_size': 0.025, 'window_stride': 0.01,
            'n_fft': 512, 'frame_splicing': 1, 'dither': 0.0,
        },
        'encoder': {
            '_target_': 'nemo.collections.asr.modules.ConformerEncoder',
            'feat_in': cfg.model.encoder.feat_in, 'n_layers': cfg.model.encoder.num_layers,
            'd_model': cfg.model.encoder.d_model, 'feat_out': -1, 'subsampling': 'striding',
            'subsampling_factor': cfg.model.encoder.subsampling_factor, 'subsampling_conv_channels': -1,
            'ff_expansion_factor': 4, 'self_attention_model': 'rel_pos', 'n_heads': cfg.model.encoder.n_heads,
            'att_context_size': [-1, -1], 'conv_kernel_size': cfg.model.encoder.conv_kernel_size,
            'dropout': 0.1, 'pos_emb_max_len': 4000,
        },
        'decoder': {
            ...
            'prednet': {
                ...
                'forget_gate_bias': 1.0, 't_max': None, 'dropout': 0.1,
            },
            'vocab_size': len(labels), 'blank_as_pad': True,
        },
        'joint': {
            '_target_': 'nemo.collections.asr.modules.rnnt.RNNTJoint',
            'jointnet': {
                'joint_hidden': cfg.model.joint.joint_hidden,
                'activation': cfg.model.joint.activation, 'dropout': cfg.model.joint.dropout,
            },
            'num_classes': len(labels), 'vocabulary': labels, 'log_softmax': True,
        },
        'decoding': {
            'strategy': 'greedy_batch', 'use_cuda_graphs': False,
        ...
        'optim': {
            'name': 'adamw', 'lr': cfg.training.learning_rate,
            'betas': [0.9, 0.98], 'weight_decay': 1e-3,
            'sched': {'name': 'CosineAnnealing', 'warmup_steps': cfg.training.warmup_steps, 'last_epoch': -1},
        },
    })
...
def find_latest_checkpoint(prefer_checkpoint: Optional[str] = None) -> Optional[str]:
    ...
    if best_path:
        print(f"Resuming from best checkpoint: {best_path.name} (run: {best_key[0]}, epoch: {best_key[1]})")
...
def load_sampling_profile(profile_name: Optional[str]) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        print(f"Warning: Could not load sampling profile '{profile_name}'. Using default. Error: {e}")
        return None
...
def main() -> None:
    ...
    if not _has_usable_cuda():
        cfg.training.accelerator, cfg.training.precision, cfg.training.num_workers = 'cpu', '32-true', 0
    else:
    ...
    model = PersonalizedRNNTModel(
        ...
        teacher_checkpoint=_resolve_path(cfg.training.teacher_checkpoint) if cfg.training.teacher_checkpoint else None,
    )

    # --- Callbacks ---
    checkpoint_callback = AnnounceCheckpoint(
        monitor='val_wer', mode='min', save_top_k=3,
        filename='epoch={epoch:02d}-wer={val_wer:.3f}', save_last=True
    )
    callbacks = [
        checkpoint_callback,
        ValidationErrorLogger(max_batches=int(cfg.validation.get('log_error_batches', 1)))
    ]
    if UNFREEZING_AVAILABLE and cfg.unfreezing.get('enabled', False):
        ...
        callbacks.append(ProgressiveUnfreezingCallback(
            unfreeze_schedule=schedule, warmup_epochs=cfg.unfreezing.get('warmup_epochs', 2)
        ))
    ...
    trainer = pl.Trainer(
        accelerator=cfg.training.accelerator, devices=cfg.training.devices,
        precision=cfg.training.precision, max_epochs=cfg.training.max_epochs,
        log_every_n_steps=20, gradient_clip_val=1.0,
        accumulate_grad_batches=cfg.training.gradient_accumulation,
        callbacks=callbacks, default_root_dir=root_dir,
