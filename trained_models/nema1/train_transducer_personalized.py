...
def find_latest_checkpoint(prefer_checkpoint: Optional[str] = None) -> Optional[str]:
    """Find the latest checkpoint, preferring most recent training date then highest epoch.
    ...
    2. Highest epoch within that date as a tiebreaker.
    """
    ...
    # Sort key: (training_date_string, epoch)
    # We want the latest date (desc) and highest epoch (desc)
    best_key: Tuple[str, int] = ("00000000_000000", -1)
    ...
    for path in candidates:
        ...
        # Compare dates lexicographically (newer is greater) and epochs numerically
        if date_str > best_key[0] or (date_str == best_key[0] and epoch > best_key[1]):
            best_key = (date_str, epoch)
            best_path = path

    if best_path:
        wer, epoch, _ = _extract_metrics(best_path)
        print(f"Resuming from best checkpoint: {best_path} (WER={wer:.3f}, epoch={epoch})")
        print(f"  Selected based on: most recent training date, then highest epoch.")
