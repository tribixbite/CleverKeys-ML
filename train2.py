...
def train_model():
    ...
    # Use local KenLM model 
    lm_path = "./wikipedia/en.arpa.bin"
    if not os.path.exists(lm_path):
        logger.error(f"KenLM model not found at {lm_path}")
        raise FileNotFoundError(f"Please download the KenLM model to {lm_path}")
    logger.info(f"Using local KenLM model: {lm_path}")
    
    # Build decoder - try with language model, fallback to without
    vocab_list = list(tokenizer.int_to_char)  # int_to_char is already a string with the vocabulary
    try:
        decoder = build_ctcdecoder(vocab_list, kenlm_model_path=lm_path, alpha=CONFIG["kenlm_alpha"], beta=CONFIG["kenlm_beta"])
        logger.info("Successfully loaded decoder with KenLM language model")
    except Exception as e:
        logger.warning(f"Failed to load KenLM model: {e}")
        logger.info("Falling back to decoder without language model")
        decoder = build_ctcdecoder(vocab_list, kenlm_model_path=None)
