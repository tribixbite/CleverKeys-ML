...
class GestureDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Optional[Dict]:
        ...
        return {
            "features": features,  # This is now already a tensor
    ...
    def _resample_features(
        ...
        """Resamples a sequence of features to a target length."""
        if features.shape[0] == target_len:
            return features

        # The feature tensor needs to be in (N, C, L) or (C, L) format for interpolate.
        # Our features are (L, C), so we transpose.
        features = features.transpose(0, 1)  # Becomes [37, 73]

        # Add a batch dimension for the interpolation function
        features = features.unsqueeze(0)  # Becomes [1, 37, 73]
        ...
        # Remove the batch dimension and transpose back
        resampled = resampled.squeeze(0)  # Becomes [37, 80]
        resampled = resampled.transpose(0, 1)  # Becomes [80, 37]
...
class GestureCTCModel(pl.LightningModule):
    ...
    def validation_step(self, batch: Optional[Dict], batch_idx: int):
        ...
        # In validation_step, replace BOTH logging blocks with this single one:

        log_interval = 5  # Log every 5 batches
        # Log if it's one of the first few batches OR if it's on the specified interval
        if (batch_idx < self.cfg.validation.log_predictions) or (
            batch_idx % log_interval == 0
        ):
            # Log up to 2 predictions from this batch
            for i in range(min(2, len(decoded_preds))):
                logger.info(
                    f"[Val Batch {batch_idx}] REF: '{words[i]:<20}' | PRED: '{decoded_preds[i]}'"
                )
